...
fully closed) and `onUserDeleted`'s server-side cleanup of
`notificationState` stay permanently inactive under this policy —
confirmed not live in production via a real authenticated
`firebase functions:list` call (403 `SERVICE_DISABLED`), not just
inferred from the decision. `functions/` remains in the repo, built and
tested, in case a future separate decision reopens this — no script
should deploy it without that. Core financial correctness is
unaffected — nothing balance/budget/report-related depends on a server.
See `docs/CLOUD_FUNCTIONS_STATUS.md` for the full record.
...
## Web security headers — deployed and live-verified (2026-08-01)

**What was done**: `firebase.json`'s Hosting config sends
`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`,
`Permissions-Policy`, and a `Content-Security-Policy` scoped to the domains
the app actually calls. Verified against *real* runtime network activity
(built the web app, served it via `firebase emulators:start --only hosting`,
...
origins the app contacts) rather than only re-reasoning from
`pubspec.yaml` — that pass caught two real gaps before ever deploying:
...
sign-in popup, not the button's own JS — would have silently broken Google
Sign-In), and that script's own font fetch
...
Both fixed. CanvasKit's `.wasm`/`.js` and the Firebase SDK bundles (all
from `gstatic.com`) were already correctly covered.

**Deployed and confirmed live 2026-08-01** — see CLAUDE.md's dated entry
for full detail, including a real, unrelated finding surfaced along the
way (the live site had been serving a real Android APK directly from
Firebase Hosting, a leftover from before this project's own free-tier
release-policy redesign, which explicitly forbids that — cleaned up as
part of this same deploy, with the owner's explicit direction). Post-
deploy, fetched the live response headers directly: `Content-Security-
Policy`, `Permissions-Policy`, `Referrer-Policy`, `X-Content-Type-Options`,
and `X-Frame-Options` all present and correct (`Strict-Transport-Security`
shows Firebase's own stronger built-in default instead, which Hosting
appears to always inject regardless of custom config — not a problem).
Loaded the live, now header-enforcing site in a real browser and confirmed
**zero CSP-violation console messages**, with every external origin from
the dry-run — including both fixed ones — loading successfully.

**Still open — not exercisable in this environment**: only the pre-auth
landing page was verified against the live, enforcing CSP. Firestore's
real `wss`/`https` connection once authenticated, Auth's own sign-in/
verification calls, and the actual Google Sign-In OAuth popup/redirect
flow completing remain unverified — this environment has no accessible DOM
to click through Flutter's CanvasKit-rendered UI, and no real Google
account to sign in with. If either flow turns out to hit a CSP violation
in real use, widen only the specific directive that failed, re-deploy, and
re-check — never widen speculatively.
...
and `flutter test` (412/412) before moving to the next: the 2 icons
already named as a specific deferred item from the Reports phase
(`reports_screen.dart`'s export button and empty-state icon —
`AppSymbols.iosShare`/`insertChartOutlined`); the 5 shared widgets
(`error_view.dart`, `sync_status_banner.dart`, `month_selector_header.dart`,
`color_picker_field.dart`, `app_password_field.dart` — 9 icons);
`home_shell_screen.dart` (23 icons — bottom-nav shell + desktop sidebar);
`financial_insight_card.dart` (14); `profile_screen.dart` (7);
`savings_goal_detail_screen.dart` (4); `savings_goals_list_screen.dart`
(4); `forgot_password_screen.dart` (2); `register_screen.dart` (2);
`account_form_screen.dart` (1); `login_screen.dart` (1);
`verify_email_screen.dart` (1). Every new `AppSymbols` constant's
codepoint was looked up directly from the installed `material_symbols_icons`
4.2960.0 package source (`symbols.dart`'s own `_rounded` definitions),
never guessed. No accessibility label, tooltip, or navigation/behavior
meaning changed anywhere in this sweep.
...
`pie_chart_rounded`/`pie_chart_outline_rounded` both resolve to `0xf0da`;
`dashboard_outline_rounded`/`info_outline_rounded` don't exist as
identifiers at all). Every icon/selectedIcon or toggle-state pair that hit
this (bottom-nav destinations, the sidebar, Savings Goals'/Accounts'
archive-filter toggle) now shares one glyph — `selectedIcon:` dropped
entirely on `NavigationDestination`s (Material 3's own selected-pill
background/icon color already communicates the active tab) and an
explicit `color: selected ? colorScheme.primary : null` tint on toggle
buttons — so no navigation-state meaning is lost, just the icon-shape
change classic Material Icons used to add on top of it. **Not covered by
any existing widget test** (no `home_shell_screen_test.dart` exists) and
